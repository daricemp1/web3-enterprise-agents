...
- **CLI Command Conventions & Flag Formats** (added 2026-07-29):
  ...
  - **`record_agent_demo.py`, `generate_demo_html.py` & `prompt_parser.py`** (added 2026-08-08, enhanced 2026-08-09): Automated demo video recording and HTML showcase generation pipeline using Playwright and FFmpeg transcoding (`-c:v libx264 -crf 22 -preset medium -movflags +faststart`). Supports `--domain <domain>`, `--name <agent_name>`, `--all`, `--speed normal|fast`, `--resolution 1080p|720p` (default: `1080p` at 1920x1080 maximized window), `--format mp4|webm`, `--url <ge_url>`, `--user-data-dir <path>` (and `CHROME_USER_DATA_DIR` for concurrent lock-free parallel execution), `--canvas-prompt <prompt>`, and `--dry-run`. Features: (1) Native 100% browser zoom enforcement (`enforce_100_percent_zoom()`), (2) 4-phase Stop-to-Action button state machine for streaming response synchronization, (3) Persistent prompt box left-click focus keeping cursor and prompt text visible during generation, (4) Tools menu -> dynamic agent-specific Turn 4 Canvas executive presentation generation (`f"Create a 4-slide executive presentation summarizing the {agent_clean_title} analysis and recommendations above."`), (5) Calibrated bottom slide thumbnail rail navigation at $Y=995$ with smooth cursor gliding (`steps=15`) and 2.5s pacing per slide, (6) Smooth left-pane full conversation scroll walkthrough, and (7) Automated HTML5 demo showcase player generation (`demos/gemini-enterprise/<domain>/<agent_name>.html`) for GitHub Pages publishing.
