...
- **CLI Command Conventions & Flag Formats** (added 2026-07-29):
  ...
  - **`record_agent_demo.py` & `prompt_parser.py`** (added 2026-08-08, enhanced 2026-08-09): Automated demo video recording pipeline using Playwright and FFmpeg transcoding (`-c:v libx264 -crf 22 -preset medium -movflags +faststart`). Supports `--domain <domain>`, `--name <agent_name>`, `--all`, `--speed normal|fast`, `--resolution 1080p|720p` (default: `1080p` at 1920x1080 maximized window), `--format mp4|webm`, `--url <ge_url>`, and `--dry-run`. Features: (1) Native 100% browser zoom enforcement (`enforce_100_percent_zoom()`), (2) 4-phase Stop-to-Action button state machine for streaming response synchronization, (3) Persistent prompt box left-click focus keeping cursor and prompt text visible during generation, (4) Tools menu -> Canvas presentation slide generation and multi-slide click-through, and (5) Smooth mouse scroll walkthrough.
