...
* **Browser Automation & Screen Capture**: Powered by **Playwright** (`playwright.chromium`), driving Google Chrome with persistent authenticated profiles, native 100% resolution enforcement, and real-time screen capture.
...
* **Sidebar Agents Discovery & Gallery Search**: Navigates via the left sidebar `Agents` tab, opens the center gallery search bar, filters by clean agent title, and selects the dedicated agent card (`x > 250, y > 180`) to open the specialized agent chat.
...
* **Two-Stage Canvas Executive Presentation**: In Turn 4, the agent generates structured 4-slide executive synthesis in dedicated chat $\to$ visual click on response Copy button (clipboard capture) $\to$ seamless "New chat" sidebar transition $\to$ Tools menu ($\mathbf{+}/$sliders) Canvas mode activation $\to$ pastes synthesis prompt and renders interactive 4-slide presentation deck on right split-pane.
* **Bottom Slide Thumbnail Rail Navigation**: Automatically navigates sequentially across all 4 generated slides via calibrated bottom thumbnail coordinates at $Y=995$ with smooth cursor gliding (`steps=15`), hover states, and 2.5s pacing per slide.
