...
- **CLI Command Conventions & Flag Formats** (added 2026-07-29, updated 2026-08-15):
  ...
  - **`record_agent_demo.py`, `generate_demo_html.py`, `generate_portal_site.py` & `prompt_parser.py`** (added 2026-08-08, enhanced 2026-08-09, updated 2026-08-15): Automated demo video recording, dual-themed HTML showcase generation, and single-page enterprise portal generation pipeline (`index.html`) using Playwright and FFmpeg transcoding (`-c:v libx264 -crf 22 -preset medium -movflags +faststart`). Supports `--domain <domain>`, `--name <agent_name>`, `--all`, `--speed normal|fast`, `--resolution 1080p|720p` (default: `1080p` at 1920x1080 maximized window), `--format mp4|webm`, `--url <ge_url>`, `--user-data-dir <path>` (and `CHROME_USER_DATA_DIR` for concurrent lock-free parallel execution), `--canvas-prompt <prompt>`, and `--dry-run`. Features: (1) Native 100% browser zoom enforcement (`enforce_100_percent_zoom()`), (2) Left-sidebar `Agents` tab navigation and center gallery search input targeting single filtered agent cards (`x > 250, y > 180`), (3) 4-phase Stop-to-Action button state machine for streaming response synchronization, (4) Persistent prompt box left-click focus keeping cursor and prompt text visible during generation, (5) Two-Stage Canvas Presentation Architecture: Turn 4 generates structured 4-slide executive synthesis in dedicated agent chat $\rightarrow$ visual click on response Copy button (clipboard capture) $\rightarrow$ seamless "New chat" sidebar transition $\rightarrow$ Tools menu ($\mathbf{+}/$sliders) Canvas mode activation $\rightarrow$ pastes synthesis prompt and renders interactive 4-slide presentation deck on right split-pane, (6) Calibrated bottom slide thumbnail rail navigation at $Y=995$ with smooth cursor gliding (`steps=15`) and 2.5s pacing per slide, (7) Smooth left-pane full conversation scroll walkthrough, (8) Automated dual-theme HTML5 demo showcase player generation (`demos/gemini-enterprise/<domain>/<agent_name>.html`), and (9) Automated single-page enterprise catalog portal generation (`index.html`) with instant search, domain filters, KPI badges, 1080p video modal player, and 1-click Light/Dark theme toggle.
