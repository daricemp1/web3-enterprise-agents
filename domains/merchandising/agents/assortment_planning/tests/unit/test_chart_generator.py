...
async def test_render_chart_returns_a_clear_error_instead_of_raising_on_bad_sql(
    ...
    # crashes the whole agent turn -- confirmed via a real deployed-agent smoke test where the
    # LLM guessed a nonexistent column and BigQuery's exception propagated uncaught.
    mock_client = MagicMock()
