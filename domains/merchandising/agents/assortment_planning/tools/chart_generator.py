...
(`ToolContext.save_artifact`). Confirmed 2026-07-26 via a real deployed-agent smoke test: the
artifact mechanism fires correctly and the resulting image renders in the real Gemini Enterprise
chat UI -- this was genuinely unconfirmed going in, given contradictory public reports about
whether Gemini Enterprise displays ADK artifacts at all. Generalized into
_shared/templates/logical_agent/ as well, so every future agent gets this capability by default.
"""
