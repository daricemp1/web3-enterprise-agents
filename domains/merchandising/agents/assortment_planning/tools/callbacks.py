...
invocation-order bug above. Injecting the project id dynamically, instead of committing it as a
literal string in the instruction, treats it the same as other deployment-specific identifiers
(service account email, Agent Engine resource name) that this repo deliberately doesn't commit --
see CLAUDE.md's "IAM is the real access boundary, not tool config" note.
"""
