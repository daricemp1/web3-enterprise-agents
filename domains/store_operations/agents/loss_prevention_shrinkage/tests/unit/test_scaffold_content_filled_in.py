...
def test_data_insights_yaml_lists_the_four_authorized_tables():
  text = (AGENT_DIR / "sub_agents" / "data_insights.yaml").read_text()
  assert "retail_ent_agents.stop_lpsh_stores" in text
  assert "retail_ent_agents.stop_lpsh_shrinkage_monthly" in text
  assert "retail_ent_agents.stop_lpsh_category_shrink" in text
  assert "retail_ent_agents.stop_lpsh_audit_exceptions" in text


def test_data_insights_yaml_does_not_hardcode_a_real_project_id():
  text = (AGENT_DIR / "sub_agents" / "data_insights.yaml").read_text()
  table_refs = re.findall(
      r"^\s*-\s+(\S+\.retail_ent_agents\.stop_lpsh_\w+)\s*$", text, re.MULTILINE
  )
...
def test_every_agent_yaml_registers_the_current_date_callback():
  for path in [
    ...
    assert (
        "loss_prevention_shrinkage.tools.callbacks.set_current_date"
        in path.read_text()
...
def test_data_insights_yaml_registers_the_bigquery_project_callback():
  text = (AGENT_DIR / "sub_agents" / "data_insights.yaml").read_text()
  assert "loss_prevention_shrinkage.tools.callbacks.set_bigquery_project" in text
