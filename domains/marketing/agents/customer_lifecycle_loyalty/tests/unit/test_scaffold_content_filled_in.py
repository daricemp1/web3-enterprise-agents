...
def test_data_insights_yaml_lists_the_four_authorized_tables():
    text = (AGENT_DIR / "sub_agents" / "data_insights.yaml").read_text()
    assert "retail_ent_agents.mktg_cloy_customers" in text
    assert "retail_ent_agents.mktg_cloy_rfm_segments" in text
    assert "retail_ent_agents.mktg_cloy_loyalty_tiers" in text
    assert "retail_ent_agents.mktg_cloy_clv_history" in text


def test_data_insights_yaml_does_not_hardcode_a_real_project_id():
    text = (AGENT_DIR / "sub_agents" / "data_insights.yaml").read_text()
    table_refs = re.findall(
        r"^\s*-\s+(\S+\.retail_ent_agents\.mktg_cloy_\w+)\s*$", text, re.MULTILINE
    )
...
def test_every_agent_yaml_registers_the_current_date_callback():
    for path in [
        ...
        assert (
            "customer_lifecycle_loyalty.tools.callbacks.set_current_date"
            in path.read_text()
...
def test_data_insights_yaml_registers_the_bigquery_project_callback():
    text = (AGENT_DIR / "sub_agents" / "data_insights.yaml").read_text()
    assert "customer_lifecycle_loyalty.tools.callbacks.set_bigquery_project" in text
